...
def run_arbitration(telemetry):
    ...
    Args:
        telemetry (dict): Performance metrics from kernel execution
    
    Returns:
        list: Constraint violations (empty if code passes)
    """
    violations = []
...
def generate_reflection_prompt(iteration, telemetry, violations):
    ...
    This prompt is what would be fed back to the LLM (or in this case, simulates
    the kind of feedback an LLM would receive and process).
    
    Args:
        iteration (int): Current iteration number
        telemetry (dict): Performance metrics from failed run
        violations (list): Constraint violations
    
    Returns:
        str: Structured reflection prompt for code correction
    """
    prompt = f"""
======================================================================
